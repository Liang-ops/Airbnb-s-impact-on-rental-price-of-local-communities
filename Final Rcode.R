...
# so most of prices in each condition are same. Though there is an increase in log median house price as condition improves, 
# we don't believe it's a good predictor.
boxplot(log(log(price)) ~ sale.condition, data = Clean.kc.housing.data, 
...
boxplot(log(price)~Logsqft_Living,data=Clean.kc.housing.data)
