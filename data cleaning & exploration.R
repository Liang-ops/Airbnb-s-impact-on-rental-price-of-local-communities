...
#I do not know what the sqft_lot15 variable is and thus will not use it since it cannot be explained in business
...
plot3=ggpairs(data=Clean.kc.housing.data, columns=c(1,10:13),axisLabels="show")
plot3
...
# Price vs. Grade ->> Nice correlation, grade increases, price increases as well
boxplot(price ~ grade, data = Clean.kc.housing.data, 
...
boxplot(price ~ sale.condition, data = Clean.kc.housing.data, 
        col=(c("grey","yellow","brown")),
        main = "Price vs. Sale.condition", xlab = "Sale.condition", ylab = "Price")
abline(h = mean(Clean.kc.housing.data$price), col = 'red')

# Price vs. Age.at.sale ->> this obviously is not a liner relationship with our target variable, at the agr between
# 40 to 60 the house price is lower than the older one or newer one, so the age cannot have big impact on the price
price_Age<- aggregate(price ~ Age.at.Sale, FUN = mean, data = Clean.kc.housing.data)
plot(price_Age,main = "Price vs. house age", xlab = "Age", ylab = "Price")

# Price vs. renovate ->> As the mean price line and the boxplot, it's obviously that whether the house renovate will have
# big affect on the sale price, the median price and max price have big different between these two situations.
boxplot(price ~ Renovate, data = Clean.kc.housing.data, 
        col=(c("grey","yellow","brown","gold")),
        main = "Price vs. Renovate", xlab = "Renovate", ylab = "Price")
abline(h = mean(Clean.kc.housing.data$price), col = 'red')

# house location exploration
# Price vs. North.South ->> We can figure out the median has large different whether the location
# is higher than the median of latitude, so it's a good predictor.
boxplot(price ~ North.South, data = Clean.kc.housing.data, 
        col=(c("grey","yellow")),
        main = "Price vs. North.South", xlab = "North.South", ylab = "Price")

# Price vs. East.West ->> We can figure out the median has no different whether the location
# is higher than the median of longitude, so it's not a good predictor.
boxplot(price ~ East.West, data = Clean.kc.housing.data, 
        col=(c("grey","yellow")),
        main = "Price vs. North.South", xlab = "North.South", ylab = "Price")

# house size exploration
# Price vs. floors ->> the number of floors appears to have little effect on house prices. 
...
boxplot(price ~ floors, data = Clean.kc.housing.data, 
        col=(c("grey","yellow")),
        main = "Price vs. Floors", xlab = "Floors", ylab = "Price")

# Price vs. Logsqft_living ->> Since the boxplot is too crowded we will plot aggregated vectors to see the relationship between 2 variables. 
# as the scatterplot shows as Logsqft_living increases, price increases as well.
price_sqftliving <- aggregate(price~Logsqft_Living, FUN=mean, data=Clean.kc.housing.data)
plot(price_sqftliving,main = "Price vs. Logsqft_Living", xlab = "Logsqft_Living", ylab = "Price")
...
price_bathrooms <- aggregate(price~bathrooms, FUN=mean, data=Clean.kc.housing.data)
plot(price_bathrooms,main = "Price vs. bathrooms", xlab = "Bathrooms", ylab = "Price")

#price vs. bedrooms --> as the boxplot shows the whole tread is when bathrooms increases then price increases as well.
#except some special situation when bathrooms equal or greater than 10.
boxplot(price ~ bedrooms, data = Clean.kc.housing.data, 
        col=(c("grey","yellow")),
